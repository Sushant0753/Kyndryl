                      ...
                      <div className="mt-3 bg-neutral-900/50 p-2 rounded-lg">
                          <audio
                            ...
                            onError={(e) => {
                              ...
                              console.error("[TTS DEBUG] Error codes: 1=ABORTED, 2=NETWORK, 3=DECODE, 4=SRC_NOT_SUPPORTED");
                              console.error("[TTS DEBUG] Failed audio URL:", msg.audioUrl);

                              // Test if URL is accessible
                              fetch(msg.audioUrl, { method: 'HEAD' })
                                .then(r => console.log("[TTS DEBUG] URL accessible, status:", r.status, "headers:", r.headers))
                                .catch(err => console.error("[TTS DEBUG] URL not accessible:", err));
                            }}
                            onPlay={() => console.log("[TTS DEBUG] Audio started playing")}
                            onCanPlay={() => console.log("[TTS DEBUG] Audio can play")}
                            onLoadStart={() => console.log("[TTS DEBUG] Audio load started")}
                            onLoadedMetadata={() => console.log("[TTS DEBUG] Audio metadata loaded")}
                          />
                      </div>
                  )}
